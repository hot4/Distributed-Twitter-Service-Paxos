...
class myThread (threading.Thread):
    ...
    def prepare(self, proposal):
        ...
        for peerPort in self.peers:
            ...
            # 1. send propose()
            c = Client("", peerPort, (site.getIndex(), site.getId()))
        ...
        if heardFromMajority == True:
            value = proposal[1][1]
            # returns true if got ACKs from everyone
            majorityAccepted = accept(site.getIndex(), value)
    ...
    def accept(self, n, v):
        for peerPort in self.peers:
            # 1. send propose()
            c = Client("", peerPort, (site.getIndex(), n, v))
            connectedClients.append(c)
